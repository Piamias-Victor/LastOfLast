...
interface GondolaPropertiesPanelProps {
  element: GondolaElement;
  updateElement: (id: string, updates: Partial<GondolaElement>) => void;
}

/**
 * Panneau pour éditer les propriétés d'une gondole/rayon
 */
const GondolaPropertiesPanel: React.FC<GondolaPropertiesPanelProps> = ({ 
  element, 
  updateElement 
}) => {
  const { id, bounds, transform, metadata, shelfCount, shelfHeight, shelfDepth, doubleSided, backgroundColor, borderColor, rackColor } = element;
  ...
  return (
    <div className="p-4">
      <h2 className="font-semibold mb-4 text-pharmacy-primary">Propriétés du Rayon</h2>
      ...
      <div className="space-y-3">
        ...
        <PropertyGroup title="Spécifications">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              />
              <label htmlFor="doubleSided" className="text-sm">
                Double face
              </label>
            </div>
          </div>
        ...
        {rackColor && (
          <ColorDisplay label="Couleur des étagères" color={rackColor} />
        )}
      </div>
    </div>
  );
...
export default GondolaPropertiesPanel;